    ...
    <section class="main" id="s1">
        ...
        if (isset($_POST['email'])) {
            ...
            if ($error == '') {
                if ($cont == 0) {
                    if ($uploadOk == 1) {
                        //Vamos a subir primero la foto.
                        if (move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)) {
                            //echo "The file " . htmlspecialchars(basename($_FILES["img"]["tmp_name"])) . " has been uploaded.";
                            $nombrearchivo='/Trabajo complementario/images/';
                            $nombrearchivo .= htmlspecialchars(basename($_FILES["img"]["tmp_name"]));
                        }
                    }
                    if (isset($nombrearchivo)) {
                        $sql = "INSERT INTO usuarios(email,nomApe,pass,tipo,Bloqueado,RutaFoto) VALUES ('$email','$nomApe','$encriptPpass', '$tipo','0','$nombrearchivo')";
                    } else {
                        $sql = "INSERT INTO usuarios(email,nomApe,pass,tipo,Bloqueado) VALUES ('$email','$nomApe','$encriptPpass', '$tipo','0')";
                    }
